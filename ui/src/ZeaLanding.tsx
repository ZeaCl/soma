    ...
    <div style={{ background: 'var(--zea-b3)', minHeight: '100vh' }}>
      ...
      <section style={{ minHeight:'90vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'80px 24px 120px', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ maxWidth:900, width:'100%', margin:'0 auto', position:'relative', zIndex:10, textAlign:'center' }}>
          ...
          <p style={{ fontSize:'clamp(1rem, 2.2vw, 1.2rem)', lineHeight:1.7, color:muted, maxWidth:650, margin:'0 auto 48px' }}>
            The infrastructure you need to operate, automate, and scale — without building it from scratch.
          </p>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(240px, 1fr))', gap:16, textAlign:'left' }}>
            {products.map(p => (
              <div key={p.name} style={{
                ...
                <div style={{ fontSize:12.5, color:muted, lineHeight:1.6 }}>{p.desc}</div>
                <div style={{ marginTop:12 }}>
                  <span style={{ padding:'2px 8px', borderRadius:6, background:p.ready ? 'color-mix(in oklch, #3fb950 10%, transparent)' : 'color-mix(in oklch, #fbbf24 10%, transparent)', color:p.ready ? '#3fb950' : '#fbbf24', fontSize:10, fontWeight:600 }}>
      ...
      <section style={{ padding:'120px 24px', borderTop }}>
        <div style={{ maxWidth:900, margin:'0 auto', textAlign:'center' }}>
          <h2 style={{ fontSize:'clamp(1.8rem, 4vw, 2.5rem)', fontWeight:700, marginBottom:48 }}>
            One platform, everything you need
          </h2>
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(250px, 1fr))', gap:24 }}>
            {reasons.map(r => (
              <div key={r.title} style={{ padding:'28px', borderRadius:14, border, background:'color-mix(in oklch, var(--zea-b2) 20%, transparent)' }}>
      ...
      <section style={{ padding:'120px 24px', borderTop, background:'color-mix(in oklch, var(--zea-b2) 30%, transparent)', textAlign:'center' }}>
        ...
        <p style={{ fontSize:16, color:muted, maxWidth:500, margin:'0 auto 40px', lineHeight:1.7 }}>
          Start deploying agents, workflows, and payments today.
        </p>
      ...
      <footer style={{ borderTop, padding:'48px 24px', color:'color-mix(in oklch, var(--zea-bc) 30%, transparent)', fontSize:13 }}>
        <div style={{ maxWidth:1100, margin:'0 auto', display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:16 }}>
          ...
          <span>AI Agents · Workflows · Messaging · Gateway · Payments</span>
        </div>
      </footer>
    </div>
  )
...
const products = [
  { icon:'🧠', name:'AI Agents', desc:'Intelligent agents that automate tasks, answer questions, and execute workflows. Deploy specialized agents for every team — finance, engineering, support, and more.', ready:true },
  { icon:'🔄', name:'Workflows', desc:'Automate complex business processes end-to-end. Approval chains, data processing, scheduled reports — run them without writing infrastructure code.', ready:true },
  { icon:'💬', name:'Messaging', desc:'Real-time communication for your platform. Channels, direct messages, @mentions, and typing indicators — integrated into your product.', ready:true },
  { icon:'🔌', name:'AI Gateway', desc:'Connect to any AI model through a single endpoint. Switch between DeepSeek, OpenAI, Claude, and more without changing a single line of code.', ready:true },
  { icon:'💳', name:'Payments', desc:'Subscription billing, invoicing, and payment processing built-in. Stripe-native, multi-currency, with analytics and reporting.', ready:false },
]

const reasons = [
  { icon:'⚡', title:'Launch in minutes', desc:'Pre-built infrastructure. No DevOps, no Kubernetes. Sign up and go live the same day.' },
  { icon:'🔒', title:'Enterprise security', desc:'Every agent runs as an isolated Linux user with kernel-level permissions. Your data stays in your organization.' },
  { icon:'📈', title:'Scales with you', desc:'Start with one agent, grow to hundreds. Same platform, same experience. No migration.' },
  { icon:'🔌', title:'Plugs into anything', desc:'REST APIs, WebSocket, SDKs, and CLI. Integrate ZEA into your existing stack in hours.' },
  { icon:'🌍', title:'Open source', desc:'Every component is open source. No vendor lock-in. Self-host or use our cloud.' },
  { icon:'🤝', title:'One partner', desc:'Agents, workflows, messaging, payments — one vendor, one support team, one bill.' },
]
