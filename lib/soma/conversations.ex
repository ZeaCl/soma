defmodule Soma.Conversations do
  ...
  def get_or_create(org_id, user_id, agent_id, app_context) do
    case Repo.get_by(Conversation, organization_id: org_id, user_id: user_id, agent_id: agent_id, app_context: app_context, is_deleted: false) do
      nil ->
        %Conversation{}
        |> Conversation.changeset(%{
          organization_id: org_id, user_id: user_id, agent_id: agent_id,
          app_context: app_context, title: "Nueva conversación"
        })
        |> Repo.insert!()
      conv -> conv
    end
  ...
  def soft_delete(org_id, id) do
    case Repo.get_by(Conversation, organization_id: org_id, id: id) do
      nil -> {:error, :not_found}
      conv ->
        Repo.update(Conversation.changeset(conv, %{is_deleted: true, deleted_at: DateTime.utc_now()}))
    end
  end
