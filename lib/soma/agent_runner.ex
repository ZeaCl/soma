defmodule Soma.AgentRunner do
  ...
  def init(opts) do
    ...
    pi_args = case File.read(config_path) do
      {:ok, content} ->
    ...
    port = Port.open({:spawn_executable, System.find_executable("sudo")}, [
      :binary,
      :stream,
      :use_stdio,
      :exit_status,
  ...
  def handle_info({port, {:exit_status, status}}, %{port: port} = state) do
    ...
    send(state.caller, {:agent_event, %{"type" => "error", "message" => "Agent process exited with code #{status}"}})
    {:stop, :normal, state}
  end
  ...
  defp handle_delta(%{"type" => "error", "reason" => reason}, state) do
    send(state.caller, {:agent_event, %{"type" => "error", "message" => "Agent error: #{reason}"}})
    state
  end
