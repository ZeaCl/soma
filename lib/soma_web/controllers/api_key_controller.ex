defmodule SomaWeb.ApiKeyController do
  ...
  post "/" do
    ...
    key_attrs = %{
      name: attrs["name"] || "default", key_hash: key_hash, key_prefix: prefix,
      scopes: attrs["scopes"] || ["soma:read", "soma:write"],
      organization_id: org_id, agent_id: attrs["agent_id"]
    }
    case %Soma.ApiKey{} |> Soma.ApiKey.changeset(key_attrs) |> Soma.Repo.insert() do
      {:ok, _} -> json(conn, 201, %{api_key: raw_key, prefix: prefix})
      {:error, cs} ->
