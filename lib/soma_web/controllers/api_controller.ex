defmodule SomaWeb.ApiController do
  ...
  post "/conversations/:id/messages" do
    ...
    case Conversations.add_message(id, attrs) do
      {:ok, msg} ->
        send_resp(conn, 201, Jason.encode!(%{data: msg}))
  ...
  post "/api-keys" do
    ...
    case %ApiKey{} |> ApiKey.changeset(key_attrs) |> Repo.insert() do
      {:ok, _} ->
        send_resp(conn, 201, Jason.encode!(%{api_key: raw_key, prefix: prefix}))
  ...
  post "/files/upload" do
    ...
    case Workspace.write_file(org_id, filepath, content) do
      {:ok, path} ->
        send_resp(conn, 200, Jason.encode!(%{ok: true, path: path, size: byte_size(content)}))
  ...
  delete "/files" do
    ...
    case Workspace.delete(org_id, path) do
      ...
      {:error, :directory_not_empty} ->
        send_resp(conn, 409, Jason.encode!(%{error: "Directorio no vacío"}))
  ...
  post "/skills" do
    ...
    case Skills.upsert(org_id, attrs["name"], attrs["content"]) do
      {:ok, skill} ->
        send_resp(conn, 201, Jason.encode!(%{data: skill}))
  ...
  put "/skills/:name" do
    ...
    case Skills.upsert(org_id, name, attrs["content"]) do
      {:ok, skill} ->
        send_resp(conn, 200, Jason.encode!(%{data: skill}))
      ...
      case Workspace.list_files(owner_type, owner_id, org_id, conn.params["path"] || "") do
        {:ok, files} ->
          ...
          send_resp(
            conn,
            200,
            Jason.encode!(%{files: result, owner_type: owner_type, owner_id: owner_id})
          )
      end
    end
  end
  ...
  delete "/sandboxes/:id" do
    ...
    case type do
      ...
      _ ->
        send_resp(conn, 400, Jason.encode!(%{error: "type must be 'user' or 'agent'"}))
    end
  end
  ...
  defp do_create_sandbox(conn, attrs) do
    ...
    if owner_id == nil || owner_id == "" do
      ...
      case type do
        ...
        _ ->
          conn |> json(400, %{error: "type must be user or agent"})
      end
    end
  end
