defmodule Soma.APITest do
  ...
  test "files CRUD flow" do
    ...
    {:ok, %{status: 200, body: body}} =
      Req.post("#{@base}/files/upload", json: %{name: "test.txt", data: b64}, headers: headers)
    ...
    # Read
    {:ok, %{status: 200, body: read}} =
      Req.get("#{@base}/files/content?path=test.txt", headers: headers)

    assert read == content

    # Mkdir
    {:ok, %{status: 200}} =
      Req.post("#{@base}/files/mkdir", json: %{path: "testdir"}, headers: headers)

    # Move
    {:ok, %{status: 200, body: body}} =
      Req.post("#{@base}/files/move",
        json: %{source: "test.txt", dest: "testdir/test.txt"},
        headers: headers
      )
    ...
    # History
    {:ok, %{status: 200, body: body}} =
      Req.get("#{@base}/files/history?path=testdir/test.txt",
        headers: headers
      )
